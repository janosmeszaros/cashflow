...
public class EditIncomeActivityTest {
    ...
    private static final Statement INCOME_STATEMENT = Statement.builder(AMOUNT, DATE).note(NOTE).type(Income).id(INCOME_ID).category(CATEGORY)
    ...
    public void testOnCreateWhenRecurringIsOtherThenNoneThenRecurringCheckBoxShouldBeCheckedAndSpinnerValueIsSettedToCorrectValue() {
        final Statement statement = Statement.builder(AMOUNT, DATE).id("12").category(CATEGORY).note(NOTE).recurringInterval(INTERVAL).type(Income)
                .build();
    ...
    public void testSubmitWhenRecurringIntervalHasChangedThenShouldCallProperFunctionAndResultCodeShouldBeOK() {
        ...
        final Statement changedStatement = Statement.builder(AMOUNT, DATE).note(NOTE).type(Income).id(INCOME_ID).recurringInterval(INTERVAL)
    ...
    public void testSubmitWhenRecurringIntervalAndDateHasChangedThenShouldCallProperFunctionAndResultCodeShouldBeOK() {
        final ShadowFragmentActivity shadowFragmentActivity = shadowOf(underTest);
        final Button submit = (Button) underTest.findViewById(R.id.submitButton);
        final Statement changedStatement = Statement.builder(AMOUNT, CHANGED_DATE).note(NOTE).type(Income).id(INCOME_ID).recurringInterval(INTERVAL)
                .category(CATEGORY).build();
        setViewsValues(changedStatement);
    ...
    public void testSubmitWhenCategoryHasChangedThenShouldCallProperFunctionAndResultCodeShouldBeOK() {
        final ShadowFragmentActivity shadowFragmentActivity = shadowOf(underTest);
        final Button submit = (Button) underTest.findViewById(R.id.submitButton);
        final Statement changedNoteStatement = Statement.builder(AMOUNT, DATE).note(NOTE).type(Income).id(INCOME_ID).recurringInterval(NONE_INTERVAL)
                .category(CHANGED_CATEGORY).build();
